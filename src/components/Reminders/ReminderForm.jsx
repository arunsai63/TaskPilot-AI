    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={submit} style={{ display:'flex', flexDirection:'column', gap:14 }}>
          ...
          <div>
            <label style={{ display:'block', marginBottom:5, fontSize:12, color:'var(--text-muted)' }}>Time *</label>
            <input className="input" type="datetime-local" value={form.time} onChange={e => set('time', e.target.value)} required />
          </div>
          <div>
            <label style={{ display:'block', marginBottom:5, fontSize:12, color:'var(--text-muted)' }}>Repeat</label>
          ...
          <div>
            <label style={{ display:'block', marginBottom:5, fontSize:12, color:'var(--text-muted)' }}>Note (optional)</label>
          ...
          <div style={{ display:'flex', gap:8, justifyContent:'flex-end', marginTop:4 }}>
            ...
            <button type="submit" className="btn btn-primary"><Bell size={14} /> {isEdit ? 'Save' : 'Set Reminder'}</button>
          </div>
        </form>
      </div>
    </div>
